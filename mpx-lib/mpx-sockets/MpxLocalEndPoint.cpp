...
MpxLocalEndPoint::MpxLocalEndPoint (MpxTaskBase* task, int endPoint, bool fast, long int timeOut, bool seqPacket) :
...
MpxLocalEndPoint::~MpxLocalEndPoint ()
{
	Release ();
}

void MpxLocalEndPoint::Release ()
{
	if (m_endPoint != 0)
		close (m_endPoint);
	m_endPoint = 0;
