...
MpxTcp4Client::MpxTcp4Client (MpxTaskBase* task, bool fast, long int timeOut, bool seqPacket) :
...
MpxTcp4Client::~MpxTcp4Client ()
{
	Release ();
}

void MpxTcp4Client::Release ()
{
	if (m_endPoint != 0)
		close (m_endPoint);
	m_endPoint = 0;
...
int MpxTcp4Client::Connect (const char* hostname, uint16_t port)
{
	if (m_IOHandle != 0)
		return 0;
...
int MpxTcp4Client::Connect (const sockaddr_in* addr, uint16_t port)
{
	if (m_IOHandle != 0)
		return 0;
